...
def get_annos(path):
    ...
    for item in my_json:
        ...
        box2d = item.get("2d_box", None)
        if box2d is None:
            xmin_2d = ymin_2d = xmax_2d = ymax_2d = -1.0  # 或 np.nan
        else:
            xmin_2d = float(box2d["xmin"])
            ymin_2d = float(box2d["ymin"])
            xmax_2d = float(box2d["xmax"])
            ymax_2d = float(box2d["ymax"])
            
        gt_boxes.append([x, y, z, l, w, h, lidar_yaw, xmin_2d, ymin_2d, xmax_2d, ymax_2d])
...
def generate_info_dair(dair_root, split):    
    ...
    for sample_id in tqdm(split_list):
        ...
        for idx in range(gt_boxes.shape[0]):
            ...
            xmin, ymin, xmax, ymax = gt_box[7:11].tolist()
            ann_info["bbox2d"] = [float(xmin), float(ymin), float(xmax), float(ymax)]
