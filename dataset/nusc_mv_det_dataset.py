...
class NuscMVDetDataset(Dataset):
    ...
    def get_gt(self, info, cams):
        ...
        for ann_info in info['ann_infos']:
            ...
            bbox2d = ann_info.get("bbox2d", [-1.0, -1.0, -1.0, -1.0])
            gt_bbox2d.append(bbox2d)
        ...
        return torch.Tensor(gt_boxes), torch.tensor(gt_labels), torch.tensor(gt_bbox2d)
    ...
    def __getitem__(self, idx):
        ...
        def _rotx(droll: torch.Tensor):
            # droll: (...)   roll, rotate around x-axis
            c = torch.cos(droll)
            s = torch.sin(droll)
            R = torch.zeros((*droll.shape, 3, 3), device=droll.device, dtype=droll.dtype)
            R[..., 0, 0] = 1.0
            R[..., 1, 1] = c
            R[..., 1, 2] = -s
            R[..., 2, 1] = s
        ...
        def _roty(dpitch: torch.Tensor):
            # dpitch: (...)  pitch, rotate around y-axis
            c = torch.cos(dpitch)
            s = torch.sin(dpitch)
            R = torch.zeros((*dpitch.shape, 3, 3), device=dpitch.device, dtype=dpitch.dtype)
            R[..., 0, 0] = c
            R[..., 0, 2] = s
            R[..., 1, 1] = 1.0
            R[..., 2, 0] = -s
        ...
        def add_yaw_pitch_roll_on_sensor2ego(
            ...
            # 先 roll 再 pitch；如果你想换顺序，也可以改成 Rx @ Ry
            Rpr = Rz @ Ry @ Rx
            ...
            T[..., :3, :3] = Rpr @ R0
            T[..., :3, 3]  = (Rpr @ t0.unsqueeze(-1)).squeeze(-1)
            return T


        def add_z_on_sensor2ego(sensor2ego: torch.Tensor, dz: torch.Tensor):
            """
            sensor2ego: (..., 4, 4)
            dz:         (...) 与 sensor2ego 的前缀维度一致（可广播）
            return: sensor2ego_noisy
            """
            T = sensor2ego.clone()
            T[..., 2, 3] = T[..., 2, 3] + dz
        ...
        # ---- sample dz ~ N(mu, sigma) on the same prefix shape ----
        mu_z = 0      
        sigma_z = 0.5

        dz = torch.randn(
            sweep_sensor2ego_mats.shape[:-2],
            device=sweep_sensor2ego_mats.device,
            dtype=sweep_sensor2ego_mats.dtype,
        ) * sigma_z + mu_z

        sweep_sensor2ego_mats = add_z_on_sensor2ego(sweep_sensor2ego_mats, dz)
        ...
        mu_yaw_rad = mu_yaw_deg * math.pi / 180.0
        sigma_yaw_rad = sigma_yaw_deg * math.pi / 180.0

        mu_pitch_rad = mu_pitch_deg * math.pi / 180.0
        sigma_pitch_rad = sigma_pitch_deg * math.pi / 180.0

        mu_roll_rad = mu_roll_deg * math.pi / 180.0
        sigma_roll_rad = sigma_roll_deg * math.pi / 180.0

        dyaw = torch.randn(
            sweep_sensor2ego_mats.shape[:-2],
            device=sweep_sensor2ego_mats.device,
            dtype=sweep_sensor2ego_mats.dtype,
        ) * sigma_yaw_rad + mu_yaw_rad

        dpitch = torch.randn(
            sweep_sensor2ego_mats.shape[:-2],
            device=sweep_sensor2ego_mats.device,
            dtype=sweep_sensor2ego_mats.dtype,
        ) * sigma_pitch_rad + mu_pitch_rad

        droll = torch.randn(
            sweep_sensor2ego_mats.shape[:-2],
            device=sweep_sensor2ego_mats.device,
            dtype=sweep_sensor2ego_mats.dtype,
        ) * sigma_roll_rad + mu_roll_rad

        sweep_sensor2ego_mats = add_yaw_pitch_roll_on_sensor2ego(
            sweep_sensor2ego_mats,
            dyaw,
            dpitch,
            droll,
        ...
        if self.is_train:
            gt_boxes, gt_labels, gt_bbox2d_raw = self.get_gt(self.infos[idx], cams)
        # Temporary solution for test.
            ida_mat = sweep_ida_mats[0, 0]  # (4,4)
            final_dim = self.ida_aug_conf['final_dim']  # (fH,fW)

            if gt_bbox2d_raw.numel() > 0:
                gt_bbox2d_aug = []
                for m in range(gt_bbox2d_raw.shape[0]):
                    gt_bbox2d_aug.append(ida_transform_bbox2d(gt_bbox2d_raw[m], ida_mat, final_dim))
                gt_bbox2d_aug = torch.stack(gt_bbox2d_aug, dim=0)  # (M,4)
            else:
                gt_bbox2d_aug = gt_bbox2d_raw.new_zeros((0,4))

        else:
            gt_boxes = sweep_imgs.new_zeros(0, 7)
            gt_labels = sweep_imgs.new_zeros(0, )
            gt_bbox2d_aug = sweep_imgs.new_zeros((0, 4))
        ...
        gt_boxes = torch.cat([gt_boxes, gt_bbox2d_aug], dim=-1)
...
def ida_transform_bbox2d(bbox2d, ida_mat, final_dim):
    """
    bbox2d: Tensor (4,) in original image coords [xmin,ymin,xmax,ymax]
    ida_mat: Tensor (4,4) for the SAME image (same cam & sweep)
    final_dim: (fH, fW) after ida augmentation
    return: Tensor (4,) in augmented coords, clipped to image bounds
    """
    if bbox2d.numel() != 4:
        return bbox2d

    xmin, ymin, xmax, ymax = bbox2d.float()
    # invalid marker
    if xmin < 0 or ymin < 0 or xmax < 0 or ymax < 0:
        return bbox2d

    # 4 corners in homogeneous 2D (x,y,1)
    pts = torch.tensor([
        [xmin, ymin, 1.0],
        [xmax, ymin, 1.0],
        [xmax, ymax, 1.0],
        [xmin, ymax, 1.0],
    ], device=ida_mat.device, dtype=ida_mat.dtype)  # (4,3)

    A = ida_mat[:2, :2]      # (2,2)
    t = ida_mat[:2, 3]       # (2,)
    xy = (pts[:, :2] @ A.T) + t[None, :]  # (4,2)
    ...
    fH, fW = final_dim
    # clip
    x_min = x_min.clamp(0, fW - 1)
    x_max = x_max.clamp(0, fW - 1)
    y_min = y_min.clamp(0, fH - 1)
    y_max = y_max.clamp(0, fH - 1)

    # guard: if collapses
    if (x_max - x_min) < 1 or (y_max - y_min) < 1:
        return torch.tensor([-1.0, -1.0, -1.0, -1.0], device=ida_mat.device, dtype=ida_mat.dtype)
